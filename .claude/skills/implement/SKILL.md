...
4. Read `.ai/plan-review-report.md`. If it exists, verify the verdict is **"Approved"**. If the verdict is "Revisions Required", **stop** and tell the developer to revise the plan via `/plan-fix` and re-run `/review-plan` first. If the file does not exist, warn the developer that the plan has not been reviewed and ask whether to proceed without review or run `/review-plan` first.
5. Check `git status`. If there are uncommitted changes unrelated to this fix, warn the developer and ask whether to proceed, stash changes, or abort.
...
Write the report to `.ai/implementation-report.md` using this exact format:
...
- After writing the artifact, inform the developer that they should review the changes and then proceed to `/code-review`.
