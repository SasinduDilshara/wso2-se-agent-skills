...
user-invocable: true
argument-hint: "(reviews current branch diff)"
---
...
4. Read `.ai/plan-review-report.md` if it exists. Use any security findings, regression risk notes, or implementation notes as additional review criteria.
5. Read `.ai/implementation-report.md` if it exists. Use it to understand what was actually implemented and any deviations from the plan.
6. Determine the base branch: check `agents.md > Contribution Guidelines > Base Branch`. If not specified, use the repository's default branch (typically `main` or `master`). Get the full diff of all changes on the current branch vs. the base branch using `git diff <base-branch>...HEAD`.
7. Verify all tests pass. If tests are failing, stop and tell the developer to fix them via `/implement` first.
...
- Verify no files were modified that aren't listed in the plan's "Files to Modify" section (unless the deviation is documented in `implementation-report.md`).
- Check for unintended file mode changes in the diff (e.g., `100644 -> 100755`).
...
- **Base branch:** [base branch used for diff]

## Verdict: Ready for PR / Changes Requested
...
- [ ] No unintended file mode changes
