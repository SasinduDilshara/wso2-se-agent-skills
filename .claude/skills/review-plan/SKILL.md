...
user-invocable: true
argument-hint: "(reads from .ai/implementation-plan.md)"
---
...
Before reviewing the plan on paper, verify its claims against the actual codebase:
- Read the source files listed in the plan's "Files to Modify" section. Verify that the files actually exist.
- Verify that the classes, methods, and APIs referenced in the plan are real and located where the plan says they are.
- Verify that the existing test suites mentioned in the plan actually exist and are runnable.
- If any references are incorrect or outdated, flag them as findings.
...
If the fix modifies shared state, concurrent data structures, or synchronization primitives, verify thread safety. Flag potential race conditions, deadlocks, or data races. This is especially important for multi-threaded server applications.

## Step 7: Regression Risk Assessment
...
## Metadata
- **Generated by:** /review-plan
- **Timestamp:** [ISO 8601 timestamp]
- **Issue:** #[ID]
- **Repository:** [owner/repo]
- **Plan version reviewed:** [version number from implementation-plan.md]
...
- **Code verification:** [Do all referenced files, classes, and methods exist?]
